...
def main():
    ...
    # totalFI,netProfitFI = loginFI(driver, urlFI, username, passwordFI) blocked
    putInFI = 990 + 280 + 0 #ETH, BTC, 0
    totalFIDollar = 7270
    totalFI = c.convert(totalFIDollar, 'USD', 'GBP')
    netProfitFI = totalFI - putInFI
    print(totalFI,netProfitFI)

    cashBlockFIDollar = 800
    cashBlockFI = c.convert(cashBlockFIDollar, 'USD', 'GBP')
    totalFI -= cashBlockFI

    #Exodus - Cardano
    print("Exodus portfolio")
    totalExodus = 1252 #maintain
    netProfitExodus = totalExodus - 1000

    #celcius - btc and usdc
    print("celsius portfolio")
    totalCashCelsius = 400 #make 1000 CASH

    #binance - AGI
    print("metamask portfolio")
    totalBinance = 369
    ...
    print("Hex")
    totalHex = 115
    netProfitHex = totalHex - 100

    #ETORO - protected
    print("Etoro protfolio")
    totalEtoro = 0
    netprofitEtoro = 0 #300 pending
    ...
    totalAssets = round(total212+totalEtoro+totalFI+metals+etfs+totalExodus+totalHex+totalBinance,3)
    netProfit = round(netProfit212+netprofitEtoro+netProfitFI+netProfitExodus+netProfitBinance,3)
    totalAssetsInvested = round(totalAssets-netProfit,3)
    percentageProfit = round(netProfit/totalAssetsInvested,5)
    netCash = round(cash212+cashBlockFI+totalCashCelsius,3)
